(() => {
  ...
  var Y = 16,
    ...
    Ce = 2,
    L = ["white", "red", "blue"],
    sk = ["pale", "peach", "yellow", "tan", "brown", "deep"],
    Pe = {
      up: { x: 0, y: -1 },
      down: { x: 0, y: 1 },
      left: { x: -1, y: 0 },
  ...
  function Ut(t) {
    return t.phase !== "playing" ? 1 : Math.min(1, t.acc / wt(t.score));
  }
  function Oe(t, e, o) {
  ...
  function Kt(t) {
    ...
    let o = e[Math.floor(t.rng() * e.length)],
      n = L[Math.floor(t.rng() * L.length)];
    t.perp = { x: o.x, y: o.y, kind: n, skin: sk[Math.floor(t.rng() * sk.length)] };
  }
  function pt(t = {}) {
    let e = {
        phase: "ready",
  ...
  function _e(t) {
    ...
    if (t.perp && o === t.perp.x && n === t.perp.y) {
      ...
      t.score += 1;
      for (let s = 0; s < be; s += 1) t.owed.push({ kind: i, skin: u });
      (t.events.push({ type: "collar", x: o, y: n, kind: i, skin: u, score: t.score }), Kt(t));
    }
  }
  function zt(t, e) {
    if (((t.t += e), t.phase === "playing"))
      for (t.elapsed += e, t.acc += e; t.phase === "playing" && t.acc >= wt(t.score);)
        ((t.acc -= wt(t.score)), _e(t));
  }
  function D(t) {
  ...
  var ot = {
    ...
    Vt = { white: "#f2f2f2", red: "#ce1125", blue: "#006847" },
    HtSkin = {
      pale: { t: "#f4d5b8", p: "#e2b894", s: "#f4d5b8" },
      peach: { t: "#e8b48c", p: "#c9926c", s: "#e8b48c" },
      yellow: { t: "#e4c06a", p: "#c9a24e", s: "#e4c06a" },
      tan: { t: "#c68642", p: "#a56b32", s: "#c68642" },
      brown: { t: "#8d5524", p: "#6e4119", s: "#8d5524" },
      deep: { t: "#5a3328", p: "#3d211b", s: "#5a3328" },
    },
  ...
  function jt() {
    ...
    for (let t of L) {
      let e = Vt[t];
      if (!e) throw new Error(`no shirt colour for perp kind "${t}"`);
      S.perps[t] = {};
      for (let n of sk) {
        let a = HtSkin[n],
          o = { ...ot, w: e, t: a.t, p: a.p, s: a.s };
  ...
  var W = 5,
    ...
    ce = {
      title: "RIO RUN",
      lines: [
        "PATROL THE LINE ALONG THE RIVER",
        "PICK UP EVERY CROSSER IN THE LOT",
  ...
  function je() {
    if (
      ...
      f.phase === "cleared" && z("Lot cleared. Every perp is in the line."),
      f.phase === "dead")
    ) {
      let t = J.value() > 0 ? `, best ${J.value()}` : "";
      z(`${f.score} deported${t}. Press space to go again.`);
      return;
    }
