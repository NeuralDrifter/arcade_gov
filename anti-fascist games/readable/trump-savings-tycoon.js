(() => {
  ...
  function P(l, e, n, t, o, r = 1) {
    let w = l.canvas.width,
      k = K(e, r);
    for (; k > w - 2 && r > 1; ) ((r -= 1), (k = K(e, r)));
    let c = Math.max(k / 2 + 1, Math.min(w - 1 - k / 2, n));
    B(l, e, Math.round(c - k / 2), t, o, r);
  }
  ...
  function fl(l, e, n, t) {
    ((l.fillStyle = "rgba(45, 120, 60, 0.75)"),
      l.fillRect(e - 10, t, n + 20, 4),
      (l.fillStyle = "rgba(45, 140, 60, 0.55)"),
      l.fillRect(e - 15, t + 4, n + 30, 4));
  }
  ...
  function se(l, { x: e, y: n, w: t, h: o, t: r, monumentX: k, monumentW: s, sunX: a }) {
    ...
    (i.addColorStop(0, "#1f4a23"),
      i.addColorStop(0.45, "#2a6a32"),
      i.addColorStop(1, "#3c8c45"),
      (l.fillStyle = i),
      l.fillRect(e, n, t, o),
  ...
  function resolveClick(l, e) {
    if (l.grabs > 0) {
      ...
      for (let n = l.bonuses.length - 1; n >= 0; n -= 1) {
        ...
        if (Me({ ...t, phase: "flying" }, e)) {
          let val = t.kind === "eagle" ? -15000 : t.value;
    ...
    if (((l.grabs -= 1), Me(l.target, e))) {
      ...
      let popText = n === "gold" ? "GOLDEN POTATOE" : null;
      return (
        (l.saved += t),
        (l.hitsThisRound += 1),
        ...
        (l.phaseT = 0),
        (l.lastResult = "hit"),
        (l.popup = { t: 0, value: t, x: l.target.x, y: l.target.y, text: popText }),
        { result: "hit", type: n, value: t }
      );
    }
    l.saved = Math.max(0, l.saved - 1000);
    l.popup = { t: 0, value: -1000, x: e.x, y: e.y, text: "MISS" };
    return { result: "miss" };
  }
  function Cl(l) {
    ((l.phase = "ready"), (l.phaseT = 0));
  ...
  var h = hl(),
    ...
    Ce = {
      title: "TRUMP CORRUPTION TYCOON",
      lines: [
        "CATCH HIS BRIBES WITH THE NET!",
        "MISSED SWINGS COST YOU $1,000",
        "OUT OF DOTS = ITEM ESCAPES",
        "3 ESCAPES AND HE GETS AWAY",
        "DON'T CATCH EAGLES OR ALIENS",
        "RECOVER STOLEN PLANES & DOCS!"
      ],
  ...
  function an(l) {
    ...
    ((h.bonusExpired.length = 0),
      h.bonusHit && ((F = { t: 0, x: h.bonusHit.x, y: h.bonusHit.y, text: h.bonusHit.text, color: h.bonusHit.color, scale: h.bonusHit.scale }), (h.bonusHit = null)),
  ...
  function wn() {
    ...
    ((u.globalAlpha = Math.max(0, 1 - l)),
      P(u, F.text || "BONUS!", x / 2, 54 - l * 12, F.color || "#ffe680", F.scale || 2),
      (u.globalAlpha = 1));
  }
  function gn() {
    let l = h.target;
  ...
  function hn() {
    ...
    if (l) {
      ...
      let textStr = l.text ? l.text : (l.value >= 0 ? `+${Y(l.value)}` : `${Y(l.value)}`);
      let col = l.value >= 0 ? "#ffe680" : "#e2564a";
      P(u, textStr, l.x + 8, l.y - 10 - l.t * 14, col);
      u.globalAlpha = 1;
    }
  }
  function pn() {
  ...
  function bn() {
    ...
    pn();
    let e = D * D * 4;
    let netRot = Math.sin(D * Math.PI) * -1.2;
    if ((bl(u, g.net, J.aim.x - 7, J.aim.y - 3 + e, netRot), D > 0.55)) {
      let o = (1 - D) * 26;
